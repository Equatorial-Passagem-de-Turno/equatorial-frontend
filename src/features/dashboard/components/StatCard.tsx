...
export const StatCard = ({ icon: Icon, label, value, color, onClick }: StatCardProps) => (
  <div 
    onClick={onClick} 
    className={`
      eq-surface-soft w-full p-4 transition-all duration-200
  ...
  >
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="text-right flex-1 min-w-0">
        <div className="text-2xl font-bold text-[var(--eq-text-primary)] truncate">{value}</div>
        <div className="text-[var(--eq-text-muted)] text-sm font-medium truncate uppercase tracking-wide">{label}</div>
      </div>
    </div>
  </div>
);
