...
export const CircuitSwitchingForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const record: CircuitSwitchingRecord = {
      id: generatedId,
      feeder,
      equipment,
      affectedCustomers,
    ...
    const previous = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]') as CircuitSwitchingRecord[];
    localStorage.setItem(STORAGE_KEY, JSON.stringify([record, ...previous]));

    setIsSubmitting(false);
    await showSuccessModal(`Circuito manobrado registrado com ID ${generatedId}.`);
    resetForm();
  };
