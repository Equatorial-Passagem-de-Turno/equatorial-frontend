...
const STORAGE_KEY = 'unavailable_equipment_records_v1';
...
const causeOptions = [
  ...
  'Defeito identificado em campo',
  'Avaria por evento externo',
  'Inspeção técnica',
...
export const UnavailableEquipmentForm = () => {
  ...
  const formTitle = useMemo(() => {
    if (equipmentNumber && equipmentType) return `${equipmentType} ${equipmentNumber}`;
    if (equipmentNumber) return equipmentNumber;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!currentDeadline) {
      await showWarningModal('Informe o prazo atual para normalização do equipamento.');
      return;
    }

    setIsSubmitting(true);

    const history = deadlineHistory.length > 0
      ? deadlineHistory
      : [{
          id: crypto.randomUUID(),
          value: currentDeadline,
          reason: 'Prazo inicial',
          registeredAt: new Date().toISOString(),
        }];

    const record: UnavailableEquipmentRecord = {
      id: generatedId,
      equipmentNumber,
      equipmentType,
      feeder,
    ...
    const previous = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]') as UnavailableEquipmentRecord[];
    localStorage.setItem(STORAGE_KEY, JSON.stringify([record, ...previous]));

    setIsSubmitting(false);
    await showSuccessModal(`Equipamento indisponível registrado com ID ${generatedId}.`);
    resetForm();
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-7 animate-fade-in relative z-10 pb-10">
      ...
      <div className="p-8 rounded-2xl border bg-purple-50/50 border-purple-100 dark:bg-purple-950/10 dark:border-purple-900/30 space-y-6 shadow-inner">
        <div className="flex items-center gap-3 mb-4 text-slate-600 dark:text-slate-300 border-b border-purple-100 dark:border-purple-900/40 pb-4">
          ...
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider">Identificação do equipamento</h3>
            <p className="text-xs text-slate-500 dark:text-slate-500">
              Priorize o número e o tipo do equipamento indisponível
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className={compactLabelClass}>Número do equipamento *</label>
            <input required type="text" value={equipmentNumber} onChange={(event) => setEquipmentNumber(event.target.value)} className={inputClass} placeholder="Ex: R-1234" />
          </div>
          <div>
            <label className={compactLabelClass}>Tipo do equipamento *</label>
            <select required value={equipmentType} onChange={(event) => setEquipmentType(event.target.value)} className={inputClass}>
      ...
      <div className="p-8 rounded-2xl border bg-slate-50 border-slate-200 dark:bg-slate-900/30 dark:border-slate-800/50 space-y-6 shadow-inner">
        <div className="flex items-center gap-3 mb-4 text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-800 pb-4">
          ...
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider">Dados operacionais</h3>
            <p className="text-xs text-slate-500 dark:text-slate-500">
              Informe localização, impacto e responsável pela normalização
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className={compactLabelClass}>Equipamento / descrição complementar</label>
            <input type="text" value={equipment} onChange={(event) => setEquipment(event.target.value)} className={inputClass} placeholder="Ex: Religador do trecho principal" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label className={compactLabelClass}>Causa da indisponibilidade *</label>
      ...
      <div className="p-8 rounded-2xl border bg-amber-50/50 border-amber-100 dark:bg-amber-950/10 dark:border-amber-900/30 space-y-6">
        <div className="flex items-center gap-3 text-slate-700 dark:text-slate-200">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Registre cada alteração de prazo solicitada pela equipe
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_auto] gap-4 items-end">
          ...
          <div>
            ...
            <input type="text" value={deadlineReason} onChange={(event) => setDeadlineReason(event.target.value)} className={inputClass} placeholder="Ex: Aguardando material em campo" />
          </div>
