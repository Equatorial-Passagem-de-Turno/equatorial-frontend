...
export const UnavailableEquipmentForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!currentDeadline) {
      await showWarningModal('Informe o prazo atual da indisponibilidade.');
      return;
    }
    ...
    try {
      ...
      const combinedObservations = `Causa: ${cause}\nDescrição: ${description}\nObs adicionais: ${observations}`;

      const formData = new FormData();
      
      formData.append('shift_id', shiftId.toString());
      formData.append('equipment_number', equipmentNumber);
      ...
      formData.append('observations', combinedObservations);
      ...
      await api.post('/unavailable-equipments', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      await showSuccessModal(`Equipamento indisponível registrado com sucesso!`);
      resetForm();

    } catch (error: any) {
      console.error('Erro ao salvar no backend:', error);
      await showWarningModal(
        error.response?.data?.message || 'Erro ao registrar o equipamento indisponível.'
      );
    } finally {
